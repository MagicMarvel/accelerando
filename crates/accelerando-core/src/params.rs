...
//! search space and to supply defaults) and is constructed from a [`Params`] map. The CLI
//! resolves a config table against the spec so that, inside [`Configurable::from_params`], every
//! declared key is guaranteed present.
...
/// hyperopt layer namespaces them across stages (e.g. `indicator.whitesnake.window`).
