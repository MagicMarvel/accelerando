...
use accelerando_core::{BrokerConfig, Params, Pipeline};
...
pub fn build_pipeline(
    ...
    let source = accelerando_sources::build(
        &cfg.data.adapter,
        &stage_overrides(&cfg.data.params, sampled, "source"),
    )
    .ok_or_else(|| format!("unknown data source: {}", cfg.data.adapter))?;

    let aggregator = accelerando_aggregators::build(
    ...
    for (i, ind) in cfg.indicator.iter().enumerate() {
        ...
        let b = accelerando_indicators::build(&ind.adapter, &stage_overrides(&ind.params, sampled, &ns))
            .ok_or_else(|| format!("unknown indicator: {}", ind.adapter))?;
        indicators.push(b);
    }

    let strategy = accelerando_strategy::build(
...
pub fn build_search_space(cfg: &RunConfig) -> Result<SearchSpace, String> {
    let mut space = SearchSpace::new();
    if let Some(s) = accelerando_sources::spec(&cfg.data.adapter) {
        space.add_spec("source", &s);
    }
    if let Some(s) = accelerando_aggregators::spec(&cfg.aggregator.adapter) {
        space.add_spec("aggregator", &s);
    }
    for (i, ind) in cfg.indicator.iter().enumerate() {
        if let Some(s) = accelerando_indicators::spec(&ind.adapter) {
            space.add_spec(&format!("indicator.{i}"), &s);
        }
    }
    if let Some(s) = accelerando_strategy::spec(&cfg.strategy.adapter) {
